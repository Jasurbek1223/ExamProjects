    ...
    where TEntity : class, IEntity where TContext : DbContext
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (cacheEntryOptions is null || !await cacheBroker.TryGetAsync<List<TEntity>>(cacheKey,
                out var cachedEntities, cancellationToken))
        {
            var initialQuery = DbContext.Set<TEntity>().AsQueryable();
            ...
            if (cacheEntryOptions is not null)
                await cacheBroker.SetAsync(cacheKey, foundEntities,
