...
public class UserInfoVerificationCodeRepository(IdentityDbContext identiryDbContext, ICacheBroker cacheBroker)
    : EntityRepositoryBase<UserInfoVerificationCode, IdentityDbContext>(identiryDbContext, cacheBroker),
        IUserInfoVerificationCodeRepository
{
    ...
    )
    {
        await DbContext.UserInfoVerificationCodes.Where(code =>
                code.UserId == verificationCode.UserId && code.CodeType == verificationCode.CodeType)
